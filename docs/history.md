# docs/history.md — the measured record behind CLAUDE.md's rules

**This file is the archive, not the rules.** Every rule that any of this
justifies lives in `CLAUDE.md`; what is here is the incident narrative, the
measurement, the superseded value and the date it changed — the evidence a rule
rests on, which is what makes the rule followable rather than arbitrary.

Nothing here was rewritten on the way in. These blocks were moved out of
`CLAUDE.md` verbatim on 2026-09-02, when that file had reached 178 KB and was
past what a session loads whole — so every session had been working from a
silently truncated copy of its own rules. **Nothing was deleted.**

Where to look for the other halves of the record:

| file | holds |
|---|---|
| `CLAUDE.md` | the rules, the Worker invariants, the active gotchas, the commands |
| `ARCHITECTURE.md` | data sources, design decisions, build position |
| `.claude/skills/worker-internals/SKILL.md` | endpoints, the KV key and TTL table, cron, external data sources |
| `.claude/skills/long-screen/SKILL.md` | Lanes A–F, move coverage, the macro regime |
| `docs/rules-evidence.md` | the measured runs behind rules 1–7 |
| `docs/failure-modes.md` | the incident record behind the named failure modes |
| **this file** | **incident narratives, superseded values, and long measurement write-ups moved out of `CLAUDE.md`** |

---

## Contents

- [Rule 1 — fan-out, `capCost`, and the `_instr` contamination rule](#rule-1-fan-out-capcost-and-the-_instr-contamination-rule)
- [Rule 2 — the `1-5` day-of-week failure](#rule-2-the-1-5-day-of-week-failure)
- [Rule 2 — why the UTC range widened 13-22 → 12-22 on 2026-09-01](#rule-2-why-the-utc-range-widened-13-22-→-12-22-on-2026-09-01)
...
- [`long:` retention, the 7:00am row sweep and the `longarch:` archive — the 2026-08-31 findings](#long-retention-the-700am-row-sweep-and-the-longarch-archive-the-2026-08-31-findings)
- [The Yahoo crumb, the fresh-`error`-row cache and the third morning firing — 2026-09-02](#the-yahoo-crumb-the-fresh-error-row-cache-and-the-third-morning-firing-2026-09-02)
- [Earnings session timing — Yahoo encodes the session as a fixed UTC anchor](#earnings-session-timing-yahoo-encodes-the-session-as-a-fixed-utc-anchor)
- [The two failure modes found building the Long tab](#the-two-failure-modes-found-building-the-long-tab)
- [A whole-object rewrite is a DELETE of every slot the writer does not own](#a-whole-object-rewrite-is-a-delete-of-every-slot-the-writer-does-not-own)
- [Worker invariants — the full annotated list](#worker-invariants-the-full-annotated-list)
- [Rule 2 — the NYSE holiday table and the early-close gap, in full](#rule-2-the-nyse-holiday-table-and-the-early-close-gap-in-full)
- [Rule 4 — the two CORS ordering rules and why curl cannot catch this](#rule-4-the-two-cors-ordering-rules-and-why-curl-cannot-catch-this)
- [Rule 5 — the seven exposed paths, the KV-write routes, and the cost denomination](#rule-5-the-seven-exposed-paths-the-kv-write-routes-and-the-cost-denomination)
- [Rule 6 — the deployed-bytes check, in full](#rule-6-the-deployed-bytes-check-in-full)
- [`printtape:` — the feature description, in full](#printtape-the-feature-description-in-full)
- [The income sleeve — the endpoint and constants description, in full](#the-income-sleeve-the-endpoint-and-constants-description-in-full)
- [`GET /api/radar` — the gates table and description, in full](#get-apiradar-the-gates-table-and-description-in-full)
- [`top3:` — the feature description and constants, in full](#top3-the-feature-description-and-constants-in-full)
- [`long:` retention and the `longarch:` archive — the constants and framing, in full](#long-retention-and-the-longarch-archive-the-constants-and-framing-in-full)
- [The frontends — the tab table, the Swing column and the card map, in full](#the-frontends-the-tab-table-the-swing-column-and-the-card-map-in-full)
- [Data: real vs. stubbed, and the POP caption](#data-real-vs-stubbed-and-the-pop-caption)
- [`return ''` in a render helper — the full audit](#return-''-in-a-render-helper-the-full-audit)
- [Appendix — the pre-compaction wording of the rules that stayed](#appendix-the-pre-compaction-wording-of-the-rules-that-stayed)

---

## Rule 1 — fan-out, `capCost`, and the `_instr` contamination rule

Justifies: *no fan-out; quote `capCost`; a per-job `_instr` from a shared firing is not a measurement; the Cache API gap.*
...
## Rule 2 — the `1-5` day-of-week failure

Justifies: *no calendar logic in the cron expression.*
...
## Rule 2 — why the UTC range widened 13-22 → 12-22 on 2026-09-01

Justifies: *check any new Pacific hour in BOTH DST regimes before scheduling it; the hour range is the only part of the expression allowed to change.*
...
## Rule 7 — `dispatchJob`, the dedup-stamp guards, the placeholder render state, and `iv:` provenance

Justifies: *every cron job goes through `dispatchJob`; a dedup stamp is guarded on the run having accomplished something; the KV stamp and a `JOB-FAILED` grep are the only cron evidence; `src` is last-writer-wins; `workersInvocationsAdaptive` is sampled.*
...
## The check suite — what each of the eighteen scripts covers, and the fixture incidents

Justifies: *run `cron-gate.check.mjs` (or a real `wrangler dev` boot) before believing `node --check`; a fixture timestamp must be relative to now; a fixture must rebuild the call's clock; never raise a tape-dependent floor to an observed total.*
...
## `printtape:` — the quarter-alignment measurement and the 2026-09-01 schedule fix

Justifies: *an actual and a consensus are compared only within one quarter; the consensus is pre-banked a session early; `tape` is a pair of windows and `usedWindow` is re-derived after every merge; `divergent: null` is a refusal.*
...
Justifies: *`income:tickers` is not a sweep universe; rows live under `incomerow:`; the fixed-rate/variable discriminator is `zeroFrac`; `ttmRate` counts payments, not days; tax character is omitted, never nulled.*
...
## `GET /api/radar` — the refusal contract, the reserved slots and the measured gate reachability

Justifies: *an unreadable watchlist REFUSES and is never cached; `candidates: null` is not `[]`; reserved slots per lane; the dollar-volume gate is a backstop, proven reachable.*
...
## `top3:` — the sequential sweep, the scoring design, and the 2026-08-31 TTL defect

Justifies: *the sweep is strictly sequential; a `status:'error'` row blocks the dedup stamp and a domain status does not; fixed anchors, never percentiles; `probMeasured` takes the MIN; one slot per ticker; `readTop3` serves the newest surviving record; `TOP3_TTL` and the walk-back are separate bounds.*
...
## `long:` retention, the 7:00am row sweep and the `longarch:` archive — the 2026-08-31 findings

Justifies: *`LONG_ROW_TTL` 7d vs `LONG_FRESH_MS` 4h are different questions; the 7am job RANKS NOTHING; the archive is cron-sweeps-only, own prefix, served verbatim.*
...
## The Yahoo crumb, the fresh-`error`-row cache and the third morning firing — 2026-09-02

Justifies: *the crumb has retention and no freshness term; every 401/403 path must force; a fresh `error` row is not a satisfied cache; the morning window is three firings.*
...
Justifies: *`earningsSession` decides a DEADLINE, not a label; the decision order is range → midnight guard → anchors → wall clock; the Yahoo field is `isEarningsDateEstimate`.*
...
## The two failure modes found building the Long tab

Justifies: *validate a vendor number against its own peers before SELECTING on it, and audit every other selector fed by the same source; guard the null before the arithmetic; a status word that cannot fire is worse than none.*
...
Justifies: *`/api/daily` is three keys merged at read time; a briefing run may clear a sibling only on a PT-date rollover, through `purgeStaleDailySlots()`.*
...
## Worker invariants — the full annotated list

CLAUDE.md keeps a compact version of every one of these. This is the annotated original, with the reasoning and the incident each bullet came from.
...
## Rule 2 — the NYSE holiday table and the early-close gap, in full

Justifies: *the gate skips NYSE holidays, not just weekends; extend `NYSE_HOLIDAYS` before the runway runs out; early closes are NOT modelled.*
...
## Rule 4 — the two CORS ordering rules and why curl cannot catch this

Justifies: *`OPTIONS` is answered before the origin 403 and before any gate; the preflight response must carry ACAO/Methods/Headers/Max-Age plus `Vary: Origin`; verify in a browser, not with curl.*
...
## Rule 5 — the seven exposed paths, the KV-write routes, and the cost denomination

Justifies: *never reintroduce a caller-supplied prompt; the gate fails closed; ceilings are denominated in Claude calls, not requests; `maySpend()` increments, so call it only once the handler knows it will spend.*
...
## Rule 6 — the deployed-bytes check, in full

Justifies: *a gate failure that survives a `DASH_KEY` edit is a deployment question first; probe with `?facts=1`; never probe with a KV-write route; both frontends carry the constant and must move together.*
...
## `printtape:` — the feature description, in full

The constants table stays in CLAUDE.md. This is the prose around it.
...
## The income sleeve — the endpoint and constants description, in full
...
## `GET /api/radar` — the gates table and description, in full

CLAUDE.md keeps the gate constants and the refusal contract.
...
## `top3:` — the feature description and constants, in full

CLAUDE.md keeps the constants table and the hard rules.
...
## `long:` retention and the `longarch:` archive — the constants and framing, in full
...
## The frontends — the tab table, the Swing column and the card map, in full

Justifies: *the Worker decides the Swing signal and ships `swingThreshold`/`swingBars`; today's forming bar is excluded until the 4:00pm ET close; card 08 is absent by design; section 07 is swing-only.*
...
> *(The earnings-session-timing block that sat here is its own section below —
> [Earnings session timing — Yahoo encodes the session as a fixed UTC anchor](#earnings-session-timing-yahoo-encodes-the-session-as-a-fixed-utc-anchor).)*
...
## Data: real vs. stubbed, and the POP caption

Justifies: *nothing is stubbed; provenance comes from `_meta`; POP is delta-derived under a lognormal assumption and debit structures render n/a.*
...
## `return ''` in a render helper — the full audit

Justifies: *a refused measurement is a FINDING, not an absence; ask whether the helper is withholding a CONTROL or a FACT.*
...
## Appendix — the pre-compaction wording of the rules that stayed

The 2026-09-02 compaction MOVED the narratives above and **restated some of the
rules that stayed in `CLAUDE.md` in fewer words**. A rewording is still a change to
the record, so every block below is the original text, verbatim, at the point it was
shortened. **The rule itself is unchanged in every case** — this exists so that
nothing from the 178 KB file is unfindable, and so a later reader can check a
compacted sentence against what it was compacted from.

Line numbers are into the 2026-09-02 pre-compaction `CLAUDE.md`.
