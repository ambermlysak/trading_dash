...
| **6** | **The daily top-3 options ranking** — `top3:{PT-DATE}`, a fourth job on the 1:15pm PT branch: a sequential watchlist sweep through the `/api/long/:ticker` path, a gated ranking over Lanes B and C directed by that day's `analysis:` verdict, and one KV read folded into `/api/long/batch`'s envelope beside `macro`. **No new endpoint and no new page-load fetch.** Every component subscore is stored, so the 0–100 score is decomposable rather than a bare number. **Serving window fixed 2026-08-26**: `readTop3` reads today's key and, on a miss, walks back up to `TOP3_SERVE_WALKBACK_DAYS` (3) calendar days — the record is written at 1:15pm PT, so today-only reading served `null` every trading morning while a valid record sat in KV. The record is served **unmodified**, with no `served` marker; `ptDate` / `asOf` date it. `/api/long/batch` is therefore `N + 2` on the hit and at most `N + 5` on the miss (measured N=2: 4 / 5 / 7) | **BUILT** 2026-08-25, serving window fixed 2026-08-26, **NOT DEPLOYED** — deploys are manual by standing rule |
