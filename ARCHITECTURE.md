...
├── swing.check.mjs      # Watchlist Swing column — the regression channel against
│                        #   two independent fits, the forming-bar drop, the x=29/30
│                        #   rule, the null path and both sides of the threshold
...
| Watchlist | 15 columns, sortable, expandable rows, one consolidated Recommendation, and the **Swing** regression-channel read | `/api/watchlist/batch` |
...
| — | **Income sleeve (`GET /api/income/*`)** | **Yahoo `quoteSummary`** (`summaryDetail` / `defaultKeyStatistics` for yield, payout ratio and the published ex-div date) **plus `/v8/finance/chart?events=div`** for the payment history, plus one spark per 20 names for the live price and the 200-day trend. Saved list in `income:tickers` — a three-field allowlist, `{ ticker, addBelow, category }`, where `category` is the Diversify tab's user-assigned enum (`income`/`cyclical`/`value`/`defensive`) — rows cached as `incomerow:{TICKER}` | **real, and MECHANICAL** — **no Claude call anywhere in the feature and no cron touches it**. Three refusals are load-bearing: `trailingAnnualDividendRate` is a **fabricated 0 on every ETF** and is nulled rather than shipped (SCHD and JEPQ both report 0 while paying), a **non-payer renders "no dividend" rather than zeros**, and a **variable distribution renders growth and cut as n/a with the reason** — decided by the measured `zeroFrac` classifier, not by a variance threshold, which was falsified. The published ex-div date ships with `exDivIsPast` and **the next one is never estimated from cadence**. **Tax character is omitted, not nulled** — it is not derivable from any Yahoo module | Nasdaq/FactSet distribution feed for declared-but-unpublished ex-div dates on funds, which Yahoo never carries |
| — | **Radar (`GET /api/radar`)** | **Yahoo predefined screeners** (`day_gainers`, `most_actives` — their rows already carry `marketCap` / price / volume / `averageDailyVolume3Month`, so the gates run with zero extra fetches) **plus the `opportunity` picks already banked in `market:sectors`**, priced by one batched `/v7/finance/quote` call. Optionability from one `/v7/finance/options` chain per finalist. Cached as `radar:{PT-date}` | **real, and MECHANICAL** — **no Claude call anywhere on the path**; every `why` string is assembled from the numbers that qualified the name, so it is a restatement of the gates rather than generated prose. Fewer than 5 is the correct output on most days and the count is whatever survives. An unreadable `watchlist:tickers` **refuses** (`candidates: null`, never `[]`) because the exclusion cannot be applied; a failing *source* is named on `sources[]` with `complete: false`, so a thin day and a broken source are distinguishable. **Scored against nothing** — it ranks by `rvol` and has not been measured against forward returns | A verified S&P 500 constituent source would unlock the v2 golden-cross sweep |
| — | **Print vs tape (`GET /api/printtape`)** | **Yahoo `quoteSummary`** — `earningsTrend.0q` for the quarter's EPS *and revenue* consensus, `earnings.earningsChart` for the actual EPS and its `periodEndDate`/`reportedDate`, `earnings.financialsChart` for actual revenue, `calendarEvents` as a consensus cross-check, and the `price` module's `postMarket*` / `preMarket*` fields for the tape — **one request per reporting name**, plus `ceil(N/20)` batched `/v7/finance/quote` calls to find who reports today. Implied move is read from the existing `long:{TICKER}` row. Banked as `printtape:{TICKER}:{ET-date}`. **Since 2026-09-01 the consensus is read a whole trading session EARLY** — a PRE-BANK pass at 13:15 PT for names reporting in `nextTradingDay(today)` — because Yahoo's `earningsTrend.0q` rolls forward once the actual is ingested and the reported quarter's REVENUE consensus then exists in no module at all. Measured: MDB's `0q` read `2026-07-31` at 20:42 UTC on 2026-09-01 and `2026-10-31` at 21:30 UTC, **48 minutes apart**. And **prior-session AMC names rejoin the next morning's BMO passes**, because both AMC passes sit inside the 90 minutes after the print while Yahoo's actuals lag it by hours to days. **Since 2026-09-03 (schema 3) the test is TWO GATES**: gate 1 is free and structured — EPS beat AND the used tape window sold it — and a name that passes is a **`candidate`**; gate 2 is the revenue beat, and its actual comes from **ONE Claude call over the news window** (`printTapeReadRelease`, which extracts the revenue figure *and* the guidance class together), with Yahoo kept as the later cross-check. MEASURED on AVGO 2026-09-02: EPS actual and both tape windows landed by the 14:30 PT pass and `revActual` was **still absent from `financialsChart` at the 06:15 PT carry-over 14 hours later** — NVDA's was absent six days on — so **the revenue half is not obtainable from Yahoo inside the window at all** and a single five-input test could only ever refuse | **real, and REFUSAL-HEAVY BY DESIGN.** An actual and a consensus are compared **only within one quarter**, by string equality on the period-end date — measured 2026-09-01, 42 min after three names reported, Yahoo had the consensus for the quarter ending 2026-07-31 and the actual for the one ending 2026-04-30, and pairing them prints a **13% MISS that never happened**. `stage` is five-valued (`not-run`/`refused`/`agree`/`candidate`/`divergent`) and `divergent` is DERIVED from it: `null` is still a refusal carrying its reason, never a "no". **A `candidate` is a FINDING, not an absence.** The release-extracted revenue passes three gates before it is believed — a null guard, a units cross-check against the model's own second statement of the figure, and a 4x plausibility band against the consensus for the same quarter — and a later Yahoo figure that disagrees by more than 1% is recorded as a `revenueConflict` with both numbers rather than overwriting. **Extended-hours VOLUME is omitted with the reason shipped** — no Yahoo field carries it and the v8 1m feed is indistinguishable from the closing auction (4 of 5 names put their whole post-window figure on the 20:00Z boundary bar at closing-auction share; pre-market read 0 on every bar for 5 of 5) | A real 8-K / press-release feed would let the release read parse the release itself rather than the news window — it is still the only revenue source inside the window, and its coverage is only as good as `newsStatus`; a logged history of outcomes would let `baseRate` stop being `not-measured` |
| — | **Swing column (Watchlist tab)** | **Computed** — OLS linear-regression channel over the last 30 **completed** daily closes from the `?range=3mo&interval=1d` chart `/api/watchlist/batch` already fetches, so **zero added subrequests**. σ is the standard error of the regression (residuals about the line), not the stdev of the closes. Today's forming bar is dropped until the 4:00pm ET close; the line is then read one bar forward, because `regularMarketPrice` belongs to today's session | **real, and INFORMATIONAL** — `swingSignal` is decided in the Worker against `SWING_Z_THRESHOLD` (1.5) and shipped decided; it sorts and gates nothing else, and **has not been scored against forward returns**. Below 30 completed bars all four fields are null and the cell renders `—` | — |
...
GET  /api/long/:ticker             One ticker (?refresh=1 forces, ?cached=1 never fetches).
                                   ...
                                   the same gate top3Sweep has always applied, added 2026-09-02 after
                                   40 of them were served as fresh for 4h. ?cached=1 is a NO-FETCH
                                   promise and still serves the error row, which renders as a finding.
GET  /api/long/:ticker?date=&slot= THE ARCHIVE READ — one banked cron-sweep snapshot from
                                   ...
                                   No Yahoo touch, no cache write, no macro read — today's regime
                                   beside a row from four sessions ago is two days' facts under one
                                   header. A miss is a 404 carrying the refusal shape and NAMING THE
                                   KEY IT PROBED, never an empty 200.
...
GET  /api/radar                    Off-watchlist discovery — <=5 quality names, no Claude call
                                     (?trail=1 emits the elimination trail, ?refresh=1 rebuilds)
GET  /api/income/list              The saved income sleeve (income:tickers), origin-gated
POST /api/income/save              Persist the sleeve — secret-gated, snapshots income:prev
GET  /api/income/batch?symbols=    Dividend rows: yield, payout, history, growth, cut, events
GET  /api/printtape?date=          PRINT vs TAPE — banked earnings-divergence records for one ET
                                     date (default today). KV assembly only, no fetches, no writes.
                                     requireSecret (x-dash-key), NOT aiGuard — it cannot spend.
                                     meta.stages groups the day's tickers by the five schema-3
                                     stages, read off the records rather than the pass entries
GET  /api/calendar/holidays?date=  NYSE full-day closures with names, plus isTradingDay /
                                     prevTradingDay / nextTradingDay for the given ET date. PURE
                                     computation: no fetches, no binding ops, nothing written.
                                     requireSecret. Exists so a consumer stops walking back by
                                     weekday — over Labor Day 2026-09-07 the session before
...
| **6** | **The daily top-3 options ranking** — `top3:{PT-DATE}`, a fourth job on the 1:15pm PT branch: a sequential watchlist sweep through the `/api/long/:ticker` path, a gated ranking over Lanes B and C directed by that day's `analysis:` verdict, and one KV read folded into `/api/long/batch`'s envelope beside `macro`. **No new endpoint and no new page-load fetch.** Every component subscore is stored, so the 0–100 score is decomposable rather than a bare number. **Serving window fixed 2026-08-26**: `readTop3` reads today's key and, on a miss, walks back calendar days — the record is written at 1:15pm PT, so today-only reading served `null` every trading morning while a valid record sat in KV. **Completed 2026-08-31**: the walk was reaching for keys retention had already thrown away, so `TOP3_TTL` went 36h → **7d** and `TOP3_SERVE_WALKBACK_DAYS` 3 → **5**, taking the unreachable probe depths from 1 to 0 and giving Monday morning (and the morning after any market holiday) a servable record for the first time. `top3sweep:last` now ages with it via `TOP3_SWEEP_STAMP_TTL` = `TOP3_TTL`. The record is served **unmodified**, with no `served` marker; `ptDate` / `asOf` date it — the aged-not-suppressed semantic `macro:state` already uses on the same envelope. `/api/long/batch` is therefore `N + 2` on the hit and at most `N + 7` on the miss (measured N=2 on 2026-08-31: 4 / 7 / 9, `extFetches 0` throughout) | **SHIPPED** — built 2026-08-25, serving window fixed 2026-08-26, TTL + walk depth completed 2026-08-31, deployed by 2026-09-01 (verified 2026-09-02: `top3:2026-09-01` expiration − write = 604800s = `TOP3_TTL` 7d, and the record exists, so the 1:15pm ranking ran) |
| **7** | **Long-row retention, the 7:00am PT row sweep, and the `longarch:` sweep archive** — one class of fix for two findings on Monday 2026-08-31. (a) `LONG_ROW_TTL` **24h → 7d**: at 24h a Friday 1:15pm write was evicted on the Saturday and no cron runs at a weekend, so the whole Options surface read “not loaded” until Monday 1:15pm. The binding case is a Thursday 1:15pm write read on the Tuesday after a Friday+Monday closure — ~115h ≈ 4.8d — plus headroom for one missed cron; the same figure and the same class of gap as `TOP3_TTL` and `MOVES_TTL`. **`LONG_FRESH_MS` stays 4h** — retention only, and verified that nothing which ranks or scores can reach an older row (`top3Sweep` reuses only inside 4h and only a non-`error` row, `top3Rank` drops any row whose own PT date is not today, `readLongRow` retires any row whose schema is not `LONG_SCHEMA`). (b) A **7:00am PT sweep-only branch** dispatching `collectMorningRows` — the same sequential watchlist sweep, so rows exist for the trading morning instead of only after 1:15pm. It **ranks nothing**: no `collectTop3`, no `top3:{PT-date}`, no `top3sweep:last`, because that dedup is a PT-date compare and a 7am stamp would silently replace the day’s post-close ranking with one priced off opening spreads. Own key `morningrows:last`; one job on the branch, so its `_instr` **is** a measurement. (c) **`longarch:{TICKER}:{PT-DATE}:{SLOT}`** — both cron sweeps bank every swept row verbatim, 7d, so a Monday-vs-Friday disagreement (Friday’s GOOGL Lane B Oct 360 call at E[R] 165% against Monday’s 14–19% on the same expiry) is a diff between two stored records rather than an argument about a row that was overwritten. Cron sweeps only, read by `GET /api/long/:ticker?date=&slot=`, and it only runs **forward** from its first deploy. Checked by `node longarch.check.mjs` (**166 comparisons at this change**; 183 after the 2026-09-01 cron rewrite and **234 after step 8 below** — the floor is always the exact count) — the only script in the suite that both extracts by source and imports the module, because half of what is at stake is structural (who may write which key) and half is routed behaviour. Both failure directions driven: reverting the TTL reddens 7 comparisons, making the 7am job stamp `TOP3_SWEEP_KEY` reddens 5 | **SHIPPED** 2026-08-31 (verified 2026-09-02: `long:AAPL` expiration − `ts` = 604800s = `LONG_ROW_TTL` 7d, where the pre-fix value was 86400s) |
| **8** | **The Yahoo crumb, the fresh-`error`-row cache, and the third morning firing** — three fixes from one measured incident, 2026-09-02 ~07:05 PT, in root-cause order. Both `morning-rows` firings failed **all 40** watchlist tickers with `Yahoo crumb unavailable (all strategies exhausted)`, banked 40 `status: 'error'` long rows (banking them **is** by design — a refusal is a finding), and every read path then served those rows as **fresh for 4h**. (a) **ROOT CAUSE — crumb retention.** `CRUMB_TTL` was 50 min in isolate memory and the `yahoo:crumb` KV copy expired at 3600s. The last writer of a trading day is the 1:15pm PT sweep, so the 7:00am PT sweep sits **17.75h downstream** and **could never reuse a crumb on any day** — it had to acquire cold at 10:00am ET, the peak of Yahoo's anti-bot pressure on datacenter IPs. **`CRUMB_KV_TTL` is now 2d and a banked crumb is reused whatever its age**, in memory and in KV: `yahooAuth` and both screener callers already re-acquire on a **401/403**, so a stale crumb self-corrects on first use while a *missing* one guarantees a cold acquire. **THE ASYMMETRY IS THE FINDING and it is not "1:15pm was fine"** — 1:15pm's own nearest predecessor is 6.25h back, which the 1h TTL also could not span, so it acquired cold too; what differed was the **hour**. A failed cold acquisition now falls back to the bank with a `console.warn` naming its age and throws only when **nothing** is banked. (b) **The latent bug longer retention armed, fixed in the same commit:** the 401/403 paths cleared `_crumbCache` but not the KV copy, so the "re-acquisition" read the same dead crumb back and retried with it. `getYahooCrumb(env, { force: true })` skips both caches and **overwrites** KV; all **three** 401/403 sites use it and nothing else may. (c) **AMPLIFIER 1 — a fresh `error` row is not a satisfied cache.** `top3Sweep`'s reuse gate has always refused them; `handleLongTicker`'s **no-param** path did not. It now falls through to the refetch. **`?cached=1` still serves the error row** — a no-fetch promise, and the row renders as the finding it is. (d) **AMPLIFIER 2 — one more morning retry.** `h === 7 && m < 30` gave two firings and both burned on the same transient fault, leaving the branch out of retries at 07:20 with the next writer six hours away. Widened to **`m < 45`** — three firings, 7:00 / 7:15 / 7:30 — a change to `scheduled()` and **not** to the cron expression, which already wakes us at `:30`. It is **no longer the same width as the 1:15pm window** (`m >= 15 && m < 45`: 30 min, two firings), and both halves are now pinned from source. Checked by `node longarch.check.mjs`, **183 → 234 comparisons** (floor raised deliberately to 234): §5g the serve rule both ways, §6f the window width re-derived from the dispatch lines, §9 the crumb. **Six failure directions driven**, and two of them are about the instrument rather than the code — §9b's crumb-value assertion *passes* under its own fault (only the fetch count catches it, because the new fallback hands the same crumb back), and §6f was **reading a comment instead of the dispatch line** until the revert was driven against it. (e) **THE STATUS WAS NEVER CAPTURED — added 2026-09-02 after the fix.** Both acquisition strategies swallowed their response (`if (r.ok)` + a bare `catch (_) {}`) and the print-tape scan discarded the error with `.catch(() => …)`, so the failure was legible only as *"all strategies exhausted"*: **the two BMO passes that day are permanently unanswerable, and no `wrangler tail` could have answered them either — the instrument was missing, not the log line.** `attempts`/`note()` carry each strategy's status into the warn and the throw, and `scanReason` in `printtapeday:{PT-DATE}` now keeps it **durably**. Diagnostic only: unconditional pushes, `note` cannot throw, nothing reads it back. **234 → 241 comparisons**, §9e/§9e2, **two further revert directions driven** — stripping the throw clause reddens 6, and dropping strategy A's non-ok capture alone reddens 2 | **SHIPPED** 2026-09-02 — crumb fix deployed 15:09:15Z (verified: `yahoo:crumb` expiration − ts = 172800s = `CRUMB_KV_TTL`, where the old code wrote 3600s). **The §(e) status capture is BUILT, NOT DEPLOYED** |
| **9** | **The print-vs-tape TWO-GATE restructure (`PRINTTAPE_SCHEMA` 2 → 3)** — from one measurement on AVGO's real 2026-09-02 AMC record, read back out of the deployed Worker on 2026-09-03 and cross-checked against `/api/earnings/AVGO?facts=1`. The EPS actual (3.32 vs 3.238) and both tape windows landed by the 14:30 PT pass; **`revActual` was still absent from `earnings.financialsChart.quarterly` at the 06:15 PT carry-over, 14 hours after a 20:00Z print**, and NVDA's was absent six days after its own. **The revenue half of the test is not obtainable from Yahoo inside this feature's window at all**, so a single five-input test could only ever return `null` — correct, and useless, on a name whose EPS and tape were fully read. (a) **TWO GATES.** Gate 1 is free and structured (EPS beat AND the used window's `changePct <= -3.0`, both out of the response the pass already fetched); gate 2 is the revenue beat. `stage` ∈ `not-run`/`refused`/`agree`/`candidate`/`divergent`, `divergent` is DERIVED from it and never assigned, and `printTapeStage` is the only decider. **The gate-1 short circuit is a LOGICAL one**: when gate 1 is fully readable and negative, the AND cannot be rescued, so the record ANSWERS — AVGO's turns from `divergent: null` into `stage: 'agree'`, its pre-market having sold **2.9871%** against a **3.00%** gate, **0.0129pp short**. **A `candidate` is a FINDING, not an absence** — on the record within 90 minutes of the print instead of behind a `null` that would not resolve for days. (b) **THE RELEASE READ supplies gate 2.** One Claude call now extracts the revenue actual *and* the guidance class together, and runs for a `candidate` as well as a `divergent` — at zero extra cost, because it is the same call, and because on the measured evidence `divergent === true` was almost never reachable: the input the verdict waited on was the input the call could have fetched. `releaseRead` stamps it, set only when the model ANSWERED, so a ceiling rejection stays retryable and an empty answer is never re-asked. **Three gates before the figure is believed** — a null guard, a units cross-check against the model's own second statement of the same number ("15.95" for $15.95bn is nine orders of magnitude and reads downstream as an ordinary miss), and a 4× plausibility band against the same quarter's consensus, which is `ivPlausible` applied to a model-extracted number. **The citation is resolved BY INDEX from the numbered coverage block, never from a URL the model wrote.** (c) **Yahoo is the LATER CROSS-CHECK, never an overwrite**: a release figure stands, a Yahoo figure past 1% becomes `revenueConflict` carrying both, and the gap-fill runs one way only. (d) **The carry-over appends its own entry to the REPORT DAY'S index whenever it SCREENED anything** (was: only when it measured), carrying `written:[tickers]` and a `scanOk` that describes the prior-index read rather than an eligibility scan it never runs — AVGO's real `printtapeday:2026-09-02` had both morning passes at `scanOk: false` and was answered the next morning, and with no entry that day reads as one whose scans simply failed. Cost re-derived in §18: a release read is **6** (4 external + 2 bindings), the heavy morning goes 53 → **113** worst case (**1.13%** of the 10,000), and **the 60-call Claude ceiling binds long before the subrequest cap**. Checked by `node printtape.check.mjs`, **543 → 862 comparisons** (floor raised deliberately to 862): §8 the two gates and the short circuit, §16 the AVGO replay reproducing the live record field by field, §17 the three validation gates, §18 the budget. **The revert direction is driven** — §16y strips the short circuit out of the shipped function's own source and reproduces the live `null` | **BUILT, NOT DEPLOYED** — all 18 check scripts green (2,647 comparisons, 0 failing) and `wrangler dev` boots it; no `wrangler deploy` has been run |
