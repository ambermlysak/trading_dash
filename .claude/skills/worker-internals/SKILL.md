...
GET  /api/printtape?date=         PRINT vs TAPE — the banked earnings-divergence records for one
                                  ...
                                  Since schema 3 `meta.stages` groups the day's tickers by
                                  `not-run`/`refused`/`agree`/`candidate`/`divergent`, read off the
                                  RECORDS rather than the pass entries — a name whose stage moved
                                  between passes would otherwise be listed under both — and each
                                  pass entry carries its own `candidates` and `written` lists.
...
- **A FAILURE CARRIES WHAT YAHOO SAID — added 2026-09-02.** Both strategies used to swallow their response (`if (r.ok)` with a bare `catch (_) {}`), so a 401, a 429, a 999 and a socket reset all produced the one sentence `Yahoo crumb unavailable (all strategies exhausted)` and **the status was captured nowhere — so a `wrangler tail` could not have reported it either.** `attempts`/`note()` append `A=<status>` / `B=<status>` / `B2=<status>` and the joined string rides on **both** the fallback `console.warn` and the throw (`… — upstream: A=429 · B=429 …`). **Diagnostic only**: unconditional pushes, `note` cannot throw, nothing reads it back as control flow. `no-status` means the response object carried no `status` — **not** a 0. The print-tape eligibility scan keeps the message rather than discarding it with `.catch(() => ({ crumb: null, cookie: '' }))`, so `scanReason` in `printtapeday:{PT-DATE}` answers *"what did Yahoo return"* **durably**, on every occurrence rather than only a watched one. §9e/§9e2 of `longarch.check.mjs` pin it (241 comparisons).
...
| `printtape:{TICKER}:{ET-DATE}` | **7d** (`PRINTTAPE_TTL`) | **PRINT vs TAPE.** One watchlist name's earnings-divergence record for one ET report date: the `print` (EPS/revenue actual vs consensus for a SINGLE named quarter), the `tape` (extended-hours price and change against the right regular close), the `implied` move read from the `long:` row, the **`stage`** (see below), the derived `divergent` verdict, and `guidance` when it fired. `PRINTTAPE_SCHEMA` **3** (2 from 2026-09-01, 1 for one day before that), **strict equality** — so every schema-2 record reads as absent, which is correct: it carries no `stage`, and a reader deriving one from `divergent` would call every unanswered name `refused` when the new code would have called most of them `agree` or `candidate`.<br><br>**SCHEMA 3 SPLITS THE TEST INTO TWO GATES — added 2026-09-03.** **GATE 1** is free and structured: EPS actual > EPS consensus (same quarter, same string-equality rule, nothing relaxed) AND the used tape window's `changePct <= PRINTTAPE_DIVERGENCE_PCT`. Both come out of the quoteSummary response the pass already fetched. **GATE 2** is the revenue beat. `stage` is one of **`not-run` · `refused` · `agree` · `candidate` · `divergent`** (`PRINTTAPE_STAGES`), with `stageReason`, and `divergent` is DERIVED from it and never assigned independently — `true` at `divergent`, `false` at `agree`, `null` elsewhere. `printTapeStage` is the only writer.<br><br>**WHY, MEASURED ON AVGO 2026-09-02 (read back out of the deployed Worker on 2026-09-03):** EPS actual 3.32 vs 3.238 and both tape windows landed by the 14:30 PT pass; `revActual` was **still absent from `earnings.financialsChart.quarterly` at the 06:15 PT carry-over, 14 hours after a 20:00Z print** — and NVDA's was absent six days after its own. **The revenue half is not obtainable from Yahoo inside this feature's window at all**, so a single five-input test can only refuse, and the refusal is correct but useless. Under the split that record is ANSWERED: gate 1 is fully readable and negative (the pre-market sold 2.9871% against a 3.00% gate, **0.0129pp short**), and the test is an AND, so no revenue figure could change it — `stage: 'agree'`, where schema 2 said `divergent: null`. **The gate-1 short circuit is a LOGICAL one, not a guess**, and it is the common case.<br><br>**A `candidate` IS A FINDING, NOT AN ABSENCE.** Gate 1 fired and gate 2 is open: an EPS beat the tape sold is exactly the shape this feature exists to surface, and it is on the record within 90 minutes of the print instead of hidden behind a `null`. It is NOT `printTapeComplete` and it DOES carry over, for a reason `printTapeNeedsCarryOver` names separately from a refusal.<br><br>**REVENUE ACTUAL FROM THE RELEASE.** When gate 1 fires, ONE Claude call (`printTapeReadRelease`, `PRINTTAPE_RELEASE_TOKENS` 700) reads the coverage window once and extracts BOTH the revenue actual and the guidance class — same call, same `AI_RATE_GLOBAL_DAY` bucket, same once-per-ticker-per-report rule, now stamped structurally by **`releaseRead`** (set only when the model ANSWERED, carried forward unconditionally by the merge, so a ceiling rejection stays retryable and an empty answer is never re-asked). The figure lands in `print.revActual` with `revActualSource: 'release-via-claude'` and its quote, currency and citation under `print.revenue`; **the citation is resolved BY INDEX from the numbered coverage block, never from a URL the model wrote**. No figure means `print.revenue = {status:'not-found-in-release', reason}` and the stage stays `candidate`. **THREE GATES BEFORE THE FIGURE IS BELIEVED:** a null/non-positive guard, a units cross-check against the model's own `revenueValueText` (`PRINTTAPE_REVENUE_CONFLICT_PCT` 1%), and a plausibility band against the consensus for the same quarter (`PRINTTAPE_REVENUE_SANITY_MULT` 4 — the `ivPlausible` rule applied to a model-extracted number, because "15.95" for $15.95bn reads downstream as an ordinary catastrophic miss).<br><br>**YAHOO IS THE LATER CROSS-CHECK, NEVER AN OVERWRITE.** When Yahoo publishes its own revenue actual days later, the merge KEEPS the release figure — the release is the company's own words, the aggregator is derived — and records `print.revenueCrosscheck`; past 1% it also records **`print.revenueConflict`** with both figures. A conflict is a finding, not something to resolve silently. The gap-fill runs one way only: a Yahoo actual that lands first is never displaced by a release read.<br><br>**SCHEMA 2 CHANGED TWO THINGS.** `tape` is now a PAIR — `pre` and `post`, each independently a reading or a refusal, with **`usedWindow`** naming which one the verdict read (the freshest by `quoteTime`, RE-DERIVED after every merge and never carried); an AMC print is traded in the post-market of its report day *and* the pre-market of the next trading day, and the second is the one that matters because Yahoo needs overnight to publish the actual. And the record gained **`consensusSource`** (`pre-banked` \| `live-pass`) and **`consensusBankedTs`**, which answer two different questions: where THIS record's figures came from, and whether a bank was taken at all. 7d retention for the same reason as `TOP3_TTL` / `LONG_ROW_TTL` — it outlives the weekend and holiday gaps the writer's trading-day schedule creates.<br><br>**`divergent` IS THREE-VALUED AND `null` IS A REFUSAL, NOT A "NO".** An unknown session, an unpublished actual or a missing consensus means the question could not be asked; answering `false` would claim it was asked. `refusalReason` always says which.<br><br>**AN ACTUAL AND A CONSENSUS ARE ONLY EVER COMPARED WITHIN ONE QUARTER**, checked by string equality on the period-end date. Measured 2026-09-01, 42 minutes after PANW/DELL/MDB reported AMC: Yahoo published the CONSENSUS for the quarter ending 2026-07-31 and, in `earningsHistory`/`earningsChart`, the ACTUAL for the quarter ending 2026-04-30. Taking the newest of each prints PANW at 0.85 vs 0.97745 — **a confident 13% MISS that never happened.** |
| `printtapeday:{ET-DATE}` | **7d** (`PRINTTAPE_TTL`) | The day INDEX: one entry per pass with its clock, the eligible names with their sessions **and their `earningsTs`**, what was measured, what was `banked` by a pre-bank pass, what was skipped and why, the scan's own ok/reason, the `divergent` AND (schema 3) `candidates` names, and — on a BMO pass — a `carryOver` block naming the prior session, who was screened, who was carried, who was `written` and who was already answered.<br><br>**THE CARRY-OVER APPENDS ITS OWN ENTRY TO THE REPORT DAY'S INDEX — widened at schema 3 from "when it MEASURED something" to "when it SCREENED anything", and AVGO's real `printtapeday:2026-09-02` is why.** Both morning passes on the report day logged `scanOk: false` (Yahoo crumb unavailable) and the record that answered the day was written the NEXT morning; a report day's index with no carry-over entry on it reads as a day whose scans simply failed. The entry carries `written: [tickers]` beside `measured`, `ranOn` naming the morning it ran, and a `scanOk` that describes the **prior-day index read** — the carry-over runs no eligibility scan at all, which is the point of reading the index instead, so hardcoding `scanOk: true` there would claim a scan that never happened. Cost: +2 binding ops on a morning that screened names and carried none. **IT IS ALSO THE CARRY-OVER'S CANDIDATE LIST**, which is why the `earningsTs` rides on it: a v7 quote row carries the NEXT earnings date and Yahoo rolls it forward within a day of a report, so a morning re-scan would find yesterday's reporters gone from the universe and report it as "nobody reported". Our own index cannot be wrong about who we measured. A pre-bank writes the index for the NEXT trading day, which that day's passes then append to. **Written on EVERY pass, a zero-eligible one included** — it is this job's dispatch evidence (rule #7), and it is what makes "nobody on the watchlist reported today" falsifiable instead of silent on the ~95% of days when that is true. It also replaces a `:last` dedup stamp, because the skip decision here is PER TICKER and reads the ticker's own record. **Outside the `printtape:` prefix**: `printtapeday:` and `printtape:` diverge at index 9 (`d` vs `:`), the same rule as `ivsweep:last` outside `iv:` and `longarch:` outside `long:`. Read by `GET /api/printtape` to answer `eligible`/`measured`/`skipped` without a fetch. |
...
**SCHEMA 3 CHANGES THE FORMULAE IN TWO PLACES** (2026-09-03, re-derived in `printtape.check.mjs`
§18): the prior-day index append now fires on **`S > 0`** rather than `C > 0`, and each **RELEASE
READ costs 6** — 4 external (`gatherEarningsFacts` issues one quoteSummary and one v8 chart
concurrently plus ONE news request, then one Anthropic call) and 2 bindings (`cronMaySpend`'s get and
put). Nothing else: the figure is folded into the record the pass was going to write anyway. A 5th
external appears only when Alpaca returns an EMPTY window and the Yahoo fallback fires.

So the formulae are **`ceil(N/20) + 4E + (S−C) + 5 + (S>0 ? 2 : 0) + 6R`** for a BMO pass,
**`ceil(N/20) + 4E + 4 + 6R`** for an AMC pass, and **`ceil(N/20) + 3E' + 4`** for the pre-bank,
which can never reach a candidate stage and so has no release term. R is the number of names that
took a release read on that pass.

**THE HEAVY MORNING, RECOUNTED.** E=10, S=12, C=8 with no candidate is still **53**; with 2
candidates **65**; with all ten a candidate — the worst case — **113**, which is **1.13%** of this
invocation's **10,000** (rule #1). A whole worst-case day across all five passes is 354, but each
pass is its own invocation and none of them sees the sum. **THE CLAUDE CEILING BINDS FIRST AND BY A
WIDE MARGIN**: `AI_RATE_GLOBAL_DAY` is **60 CALLS** a day across every path, request and cron alike,
so twenty release reads is a third of the day's budget — rule #5's "ceilings are denominated in
Claude calls, not requests" is the constraint here, not the subrequest cap. A refused call degrades
the record (the gate-1 candidacy stands, only the release read is skipped) and never blocks it. The
one figure that MOVED with no release read at all: a morning that screens names and carries none went
from **14 to 16**, the +2 being the prior-day append that makes "the carry-over ran and found
everything answered" falsifiable.
...
**Zero Claude calls unless a name reaches GATE 1 — a `candidate` or a `divergent` (schema 3; it was
`divergent` alone before, which the measured evidence says was almost never reachable, since the
revenue actual Yahoo needs days to publish was the very input the verdict was waiting on).** One call
per ticker per report, against the same `AI_RATE_GLOBAL_DAY` bucket the request paths use
(`cronMaySpend`, which REFUSES on a KV failure where `aiGuard` proceeds — a cron has no second
control). It costs nothing extra to run it on candidates: it is the SAME call, and a candidate the
call turns into a divergent has spent one in total, not two. **The release read is the only path that
can satisfy gate 2 inside the window**, so refusing it to a candidate would leave the feature unable
